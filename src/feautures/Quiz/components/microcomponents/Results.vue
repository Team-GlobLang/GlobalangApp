<template>
    <div class="p-6 bg-white rounded-2xl shadow-lg max-w-md mx-auto mt-10 text-center">
        ...
        <div class="flex justify-center gap-4 mb-6">
            <FwbButton @click="repeatQuiz" class="">
                🔄 Repeat Quiz
            </FwbButton>
            <FwbButton @click="exitQuiz" color="dark">
                🚪 Exit
            </FwbButton>
        </div>
        ...
        <div class="mt-4">
            ...
            <div v-if="showReview">
                <div v-for="(q, index) in questions" :key="q.id" class="mb-4 p-4 bg-gray-50 rounded-lg shadow">
                    <h3 class="font-semibold">{{ index + 1 }}. {{ q.questionText }}</h3>
                    <ul class="list-disc list-inside mt-1">
                        <li v-for="opt in q.options" :key="opt.id" :class="{
                            'text-green-700 font-semibold': opt.isCorrect,
                            'text-red-600 line-through': userAnswers[index]?.id === opt.id && !opt.isCorrect
                        }">
                            {{ opt.text }}
                        </li>
                    </ul>
