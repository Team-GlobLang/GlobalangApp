<template>
  <div
    class="bg-white shadow p-4 rounded-2xl flex flex-col justify-between h-full transition hover:shadow-lg"
  >
    ...
    <div class="flex justify-between items-end w-full">
      ...
      <div class="flex gap-3">
        <FwbButton outline color="red" @click="onRemoveQuiz">
          Delete
        </FwbButton>
        <FwbButton color="green" @click="onReviewQuiz"> Review </FwbButton>
      </div>
    </div>
  </div>
</template>
...
const emit = defineEmits<{
  (e: "review", quizId: string): void;
  (e: "remove", quizId: string): void;
}>();
