<template>
    <div class=" w-full flex flex-col items-center">
        ...
        <div class=" w-11/12 bg-white py-3 shadow rounded-2xl flex flex-col items-center">
            <div class=" w-full" v-if="currentStep === 0">
                <CreateQuizConfigurationFom ref="configRef" v-model="quizData.quiz" />
            </div>
            <div class=" w-full" v-else>
                <CreateQuestionForm v-if="questions[currentStep - 1]" ref="questionRef" :current-step="currentStep"
                    v-model:question="questions[currentStep - 1]" @delete-question="deleteCurrentQuestion" />
            </div>

            <QuizStepperButtons :currentStep="currentStep" :prevStep="prevStep" :saveConfig="saveConfig"
                :next-step="nextStep" :submitQuiz="submitQuiz" :isPending="isPending"
...
const submitQuiz = async () => {
    ...
    mutate(quizData.value, {
        onSuccess: () => { router.push({ name: "QuizStudio" }) }
    })
...
const breadcrumbItems = computed(() => {
    ...
    if (currentStep.value === 0) {
        items.push({ label: "Configuration", route: "/studio/create-quiz" });
    } else {
        items.push({ label: `Question #${currentStep.value}`, route: "/quiz-studio/create-quiz" });
    }
    return items;
});
